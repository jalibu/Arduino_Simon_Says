...
#define ROUNDS_TO_WIN      9999 //Number of rounds to succesfully remember before you win. 13 is do-able.
...
int highScore = 0;
boolean hurryUpNotified = false;

void setup()
{
  Serial.begin(9600);
  Serial.print("EPROM " + String(readIntFromEEPROM(EEPROM_ADDR)));
  highScore = readIntFromEEPROM(EEPROM_ADDR);
  if(highScore == 255)
    highScore = 0;
  ...
  Serial.print("Game mode " + gameMode);
...
boolean play_memory(void)
{
  ...
  {
    clearAndWriteLcd(0, "Viel Gl\365ck");
    hurryUpNotified = false;
    add_to_moves(); // Add a button to the current moves, then play them back

    clearAndWriteLcd(1, "Runde " + String(gameRound));
    ...
    for (byte currentMove = 0 ; currentMove < gameRound ; currentMove++)
    {
      ...
      if (choice != gameBoard[currentMove])
      {
        if(gameRound -1 > highScore){
          ...
          clearAndWriteLcd(0, "Neuer Highscore!");
          writeIntIntoEEPROM(EEPROM_ADDR, highScore);
          play_winner();
        } else {
          clearAndWriteLcd(0, "Verloren!");
        }
...
byte wait_for_button(void)
{
  ...
  {
    if(!hurryUpNotified && millis() - startTime > ENTRY_TIME_LIMIT / 2){
      hurryUpNotified = true;
      clearAndWriteLcd(0, "Beeil dich!");
    }
...
{
  clearAndWriteLcd(0, "Los gehts!");
  
  clearAndWriteLcd(1, "Highscore: " + String(highScore));
  while(1) 
  {
    setLEDs(CHOICE_RED);
    delay(100);
...
void clearAndWriteLcd(int line, String text)
{
